...
- `POST /api/me/accounts/:id/import-all-history` is an explicit Lichess-only action that queues the full supported game range from `2013-01-01` (the provider's earliest export boundary) through the request time. It remains a durable background run and keeps the normal standard Bullet/Blitz/Rapid, rated-and-casual scope.
- When the importing app user has an active Lichess OAuth connection, the worker uses that user's bearer token for public game-export requests, including imports for other tracked public usernames. Expired, revoked, or unavailable credentials fall back to the anonymous public endpoint; private games are not requested, and credentials are never shared between app users.
...
Rollback must preserve already accepted durable work and the persisted source vocabulary used by that work. If durable imports have been accepted, keep the compatible worker running until those imports and linked preparation work are terminal or explicitly paused/cancelled. Draining active work is necessary before an application rollback, but it is **not sufficient** to deploy a binary/contracts revision that predates the persisted `ACCOUNT_REFRESH` source: terminal import rows are intentionally retained and older contracts do not recognize that source value. Roll back only to a revision that remains schema- and source-compatible with retained durable rows. Do not roll the API back to the former synchronous-provider implementation while durable work for the same accounts is active. The old Angular account page is also incompatible with the new `202` response shape, so API and web rollback must remain coordinated. Database migrations are forward-compatible persistence for retained import/preparation history and should not be reverted as an application rollback mechanism; any deliberate rewrite/removal of retained source data would require a separately reviewed migration, not an operational rollback shortcut.
