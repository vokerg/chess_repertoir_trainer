...
Imported-game analysis keeps reusable engine output and per-game classification separate. Reusable position analysis is stored in the analysis module's position-analysis cache with compact or rich persistence: imported-game flows write scalar-only compact rows, while free/interactive analysis can write rich rows with PV lines. Per-game ply score loss and classification fields are stored on `ImportedGamePly` in batches. `ImportedGameAnalysisService` is the direct, awaitable single-game execution boundary used by the persistent worker. See [Position Analysis Cache](position-analysis-cache.md) for browser interactive analysis and backend whole-game analysis flows.

Imported-game ply moves use the stateless `chess-domain` UCI codec for `SMALLINT` storage. Persistence stores required `moveCode` values; readers and aggregations use codes internally and return UCI strings at repository boundaries. The [rollout and restoration procedure](operations/imported-ply-move-codes.md) describes the verified backfill, removal of the legacy column/index, and optional recovery of exact UCI strings through the same domain codec. Course move nodes and tactical detection records retain their UCI storage.
...
`apps/api/src/worker.ts` is a separate process entry point. It loads worker timing and retention configuration, removes expired terminal job history, creates the imported-game executor registry, starts the jobs-module scheduling loop, handles `SIGINT`/`SIGTERM`, aborts the active executor through an `AbortSignal`, waits for bounded graceful shutdown, and disconnects its own Prisma client. The API process does not start or own this loop.
...
- Course content carries a monotonic revision and content-change timestamp. Each logical bundle mutation increments the affected course once; moving content between courses increments both. Training and statistics writes do not change content revisions. Course identity also persists one primary training side and an optional curated cover key; changing either is a content revision, while the underlying lines keep their own authoritative `sideToTrain` values.
...
- Marathon modes are `ALL`, `WEAK_SUBLINES`, `UNTRAINED_SUBLINES`, `MIXED_WEAK_UNTRAINED`, and web/API-only `DAILY_REVIEW`. Weak and untrained filtering uses the same active subline extraction and last-5 scored-attempt window as stats.
- Daily Review selects only active canonical sublines with an owned `RepertoireSublineReviewState` due at or before the run's start. Focused line training and the ordinary marathon modes seed a stage-0 review without overwriting an existing schedule: passed material is due the following day, while newly failed material is due immediately. Clean scheduled attempts advance through the fixed 1/3/7/14/30/60-day ladder, failed scheduled attempts reset to stage 0 and remain due tomorrow, and one short-lived reinforcement retry is queued behind untouched due work without changing the persistent date.
- The web marathon flow creates a bounded, short-lived in-memory run and requests subsequent candidates by run id. Prepared candidate trees, recent hashes, and Daily Review retry state live for 30 minutes of inactivity, are ownership checked, and do not survive API restarts, matching active training-session lifetime. Daily Review returns an explicit completed response after due items and retries are exhausted; the legacy stateless `/api/training-marathons/next` route remains compatible with the four random modes.
...
- `/library/marathon`, `/courses/:courseId/marathon`, and `/chapters/:chapterId/marathon` share the line marathon page. Query parameters can provide `lineIds`, `sublineHashes`, and `mode`; route params provide optional course/chapter scope. Daily Review completion offers an in-place transition to the ordinary `ALL` mode with the same scope.
