...
Imported-game ply moves use the stateless `chess-domain` UCI codec for `SMALLINT` storage. During the [staged rollout](operations/imported-ply-move-codes.md), persistence dual-writes nullable `moveCode` and legacy `moveUci`; readers and aggregations use codes internally and return UCI strings at repository boundaries. The legacy column and composite index remain until production backfill validation and a separate contract migration. Course move nodes and tactical detection records retain their UCI storage.
