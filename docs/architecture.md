...
- Daily Review selects only active canonical sublines with an owned `RepertoireSublineReviewState` due at or before the run's start. Focused `LINE` training deterministically seeds a stage-0 review for the following day; random marathon modes neither seed nor mutate review scheduling. Clean scheduled attempts advance through the fixed 1/3/7/14/30/60-day ladder, failed attempts reset to stage 0 and remain due tomorrow, and one short-lived reinforcement retry is queued behind untouched due work without changing the persistent date.
