...
- Opening struggles selects only early plies, per-move score loss, and the compact position-evaluation scalars used for analysis links and response context. It also loads all user-owned course lines in one separate course-content query, builds one repertoire graph per course and trained side, and annotates returned prefixes without per-row database calls.
