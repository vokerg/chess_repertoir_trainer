...
`SKIPPED` dismisses first-run guidance but does not make an already active preparation disappear: a skipped user with accepted background work receives valid controls for that run and is never told to start a second concurrent preparation. Skipped recovery/attention states expose operational recovery choices, not another finish/skip decision. A skipped user with no accepted preparation history may start onboarding; a terminal historical run is handled according to its restart/re-entry state.
