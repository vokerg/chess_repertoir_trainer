...
The readiness endpoint itself remains read-only. ONB-009 implements the authenticated mutations under `/api/me/onboarding`: first-run start/skip, finishable-attention completion, run pause/resume/cancel/retry, terminal restart, and immutable expansion. Those commands consume this projection's action vocabulary; they do not move lifecycle authority into the browser or duplicate the preparation reconciler.
