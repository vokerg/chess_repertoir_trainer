...
ONB-009 authenticated commands consume the existing preparation pause/resume/cancel/retry boundary without duplicating the reconciliation state machine. The command layer additionally coordinates recoverable linked-import attention: `IMPORT_PAUSED` resumes the durable linked import and lets the persisted wake path recover the parent, while `IMPORT_RETRY_AVAILABLE` creates an immutable import retry and atomically relinks the existing preparation target. ONB-008 owns user disposition, readiness/presentation projection, and action mapping. ONB-015 owns account-sync/preparation import handoff and can set `currentImportRunId`; recoverable import attention observes that persisted handoff through the database wake path. ONB-019 owns destructive resource fences, and onboarding/import/preparation command admission enters those canonical guards rather than creating a second lifecycle state machine.
