...
Tactical detections are a Lab-only persisted report over already analysed imported games. The feature does not run an engine and does not create puzzle or training sessions. It reads cached `PositionAnalysis` rows and `ImportedGamePly` rows from games whose `ImportedGame.latestAnalysisStatus` is `COMPLETED`.
