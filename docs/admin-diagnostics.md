...
Execution requires the preview token, exact typed confirmation, a stable idempotency key, normal authenticated administrator access, and the server-side capability check. `PURGE_ACCOUNT_DATA` is the deliberately small administrator path: it takes the user lock, temporarily owns the account fence, retires competing lifecycle work for that user, purges the account in one transaction, and returns the existing operation as `COMPLETED`. The other account/game actions retain the worker-backed lifecycle path. Administrator executions record that the typed confirmation phrase was used; no separate email challenge or fresh reverification token is required.
