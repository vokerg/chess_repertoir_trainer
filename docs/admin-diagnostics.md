...
Execution requires the preview token, exact typed confirmation, a stable idempotency key, normal authenticated administrator access, and the server-side capability check. `PURGE_ACCOUNT_DATA` is the deliberately small administrator path: it takes the user lock, revalidates the exact preview, installs a durable account fence, retires competing lifecycle work for that user, cancels and drains active import/preparation/job work, then purges the account in one destructive transaction and returns the existing operation as `COMPLETED`. The other account/game actions retain the worker-backed lifecycle path. Administrator executions record that the typed confirmation phrase was used; no separate email challenge or fresh reverification token is required.

The web UI binds lifecycle actions to visible account entities. The account settings cards expose self-service purge and connected-account deletion; administrator diagnostics selects an account from the selected user's bounded account list and displays its account ID for auditability. Neither surface accepts a manually copied target ID as the primary selection mechanism. Both use the same exact-count preview, typed phrase, status refresh, and stop controls.
