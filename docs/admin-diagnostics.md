...
Execution requires the preview token, exact typed confirmation, a stable idempotency key, normal authenticated administrator access, and the server-side capability check. The API rechecks the preview digest, ownership, exact aggregate counts, lifecycle state, and idempotency binding before installing the resource fence. Administrator executions record that the typed confirmation phrase was used; no separate email challenge or fresh reverification token is required.
