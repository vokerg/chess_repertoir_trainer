...
- Rich rows store scalar best move/eval and up to three engine lines/PVs. Interactive/free/opening/line-editor board analysis requests rich depth 18 and MultiPV 3. Rich rows are considered reusable for interactive navigation when the best line is depth 17 or deeper and the required legal PV lines are present.
...
The frontend routes position analysis through `PositionAnalysisCacheService`, which owns the analysis profiles and cache requirements. Feature stores request analysis by intent: interactive board features call the rich interactive path, while imported-game analysis calls the compact game path.
...
- Depth 12 rich rows do not satisfy depth 18 interactive requests; the browser reruns Stockfish and rich-saves upgraded depth 18 lines. Rich rows whose best line is depth 17 or depth 18 and whose required legal PV lines are present are reused.
