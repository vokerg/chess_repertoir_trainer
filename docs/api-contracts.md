...
`packages/contracts` is an active root workspace consumed by API and client build graphs. Its verified public exports include imported-game browsing, persistent imported-game jobs, board-image endpoints, course read models, serializable training, training statistics and training-marathon responses, mobile synchronization, Lab performance-by-rating and top-opponents reports, opening-struggles queries/responses, repertoire-target definitions, repertoire-coverage course review responses, Lichess connection and bot-challenge responses, and Lichess puzzle rounds. Add schemas only after checking the real service output and consumers; unverified provisional exports are not acceptable.
