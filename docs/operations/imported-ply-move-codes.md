...
On 2026-09-26, the configured Neon database contained 882,113 plies. After temporarily dropping the new code index to free approximately 62 MB, the resumable run filled the remaining 156,113 codes and validated **all 882,113 rows with zero nulls and 100% exact UCI equality**. The database was approximately 435 MB before database-wide vacuum. The contract was then applied and recorded after repeating exact validation under an exclusive table lock. Database-wide `VACUUM ANALYZE`, physical compaction of the ply table, and restoration of its equivalent move-code index reduced the database from approximately 490 MB to **356 MB**. Both remaining ply indexes are valid. A final 882,113-row decode/export matched the original compressed recovery copy byte-for-byte (SHA-256 `1ecd15c3ab08f2f75a882b294329ae26beb3cb29cbd7950b2e3a51bc0d4e052e`). A table audit found no confirmed unused tables; no live tables were dropped. The deployed API still requires this code-only release; no deployed worker was present.
