...
assert.equal(e4.evidence.course.conflict, false);
assert.equal(e4.evidence.course.transposesToCoveredPosition, false);
assert.equal(e4.evidence.opening.knowledge.plans[0].id, 'sharp-test-plan');
assert.equal(e4.reasonCodes.includes('TRANSPOSES_TO_COVERAGE'), false);
assert.equal(e4.warningCodes.includes('COURSE_CONFLICT'), false);
