...
try {
  ...
  const masters = await upsertMastersExplorerCache({
    ...common,
    source: 'LICHESS_MASTERS',
    topGamesLimit: 15,
  });
  const population = await upsertMastersExplorerCache({
    ...common,
  ...
  const foundMasters = await findMastersExplorerCache(normalizedFen, 'LICHESS_MASTERS', 1);
  const foundPopulation = await findMastersExplorerCache(normalizedFen, 'LICHESS_GAMES', 1);
  ...
  console.log('Lichess games explorer repository tests passed.');
} finally {
  await cleanup();
