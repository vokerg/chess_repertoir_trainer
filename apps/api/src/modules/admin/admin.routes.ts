...
const adminModule: FastifyPluginAsyncZod<AdminModuleOptions> = async (app, options) => {
  ...
  app.route({
    ...
    schema: {
      ...
      description: 'Validates the preview token, typed confirmation, and idempotency key. PURGE_ACCOUNT_DATA completes during the request after durable fencing and account-work drain; other account/game actions continue through the lifecycle worker.',
