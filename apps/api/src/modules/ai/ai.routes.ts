...
const aiModule: FastifyPluginAsyncZod = async (app) => {
  ...
  app.post('/api/imported-games/:gameId/ai-review', {
    schema: {
      ...
      description: 'Bodyless on-demand action using persisted game metadata and completed engine analysis.',
    ...
    try {
      ...
      return reply.code(mapped.statusCode as 404 | 409 | 429 | 502 | 503 | 504).send({
        code: mapped.code,
