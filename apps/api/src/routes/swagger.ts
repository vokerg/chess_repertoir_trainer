  ...
  paths: {
    ...
    '/api/me/accounts/{id}/reset-cursor': {
      post: {
        ...
        description:
          'Clears the account sync cursor so the next sync re-scans the full provider history for that account. Previously imported games remain protected by per-account provider game ids and are skipped rather than duplicated.',
        parameters: [{ $ref: '#/components/parameters/AccountId' }],
        responses: {
          '200': {
            description: 'Updated external account',
    ...
    schemas: {
      ...
      ImportRunSummary: {
        ...
        properties: {
          ...
          gamesUpdated: { type: 'integer', description: 'Legacy field kept for compatibility. Existing games are no longer rewritten during sync and this value should remain 0.' },
          gamesSkipped: { type: 'integer', description: 'Games seen but not imported because they were already present or outside the incremental sync window.' },
