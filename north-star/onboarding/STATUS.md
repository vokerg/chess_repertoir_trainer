...
- ONB-026: implementation/review fixes are on PR #412; the migration is applied against PostgreSQL `server_version_num=170011`, and the focused trigger, concurrency, lifecycle, worker, command-orchestration, and bounded benchmark suite passed. The latest benchmark observed transaction p90 `325.32ms` and lock p90 `157.52ms`, below the accepted `1000ms`/`250ms` limits. A full production-scale manual sweep was intentionally not run against the shared ~1.07M-position database; the task report records that residual and the unrelated full-API shared-state test failure.
